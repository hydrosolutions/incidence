...
def partition(values):
    return {
        ...
        "partition": {
            ...
            "branches": [{"branch": b, "expression": e} for b, e in values.items()],
        },
    }
...
def document(initial, incoming, outgoing, quanta, steps=1):
    ...
    bindings = []
    for s in substances:
        for node, branches in [
            ("supply", {"pool": incoming[s]}),
            ("pool", outgoing[s]),
        ]:
            exprs = {}
    ...
    return i.model_document(
        finite_compartments=["supply", "pool"],
        boundary_accounts=["left", "right"],
        connections=[
            {"source": "supply", "target": "pool"},
            {"source": "pool", "target": "left"},
            {"source": "pool", "target": "right"},
        ],
        ...
        initial_stocks=[
            {
                ...
                "amounts": [
                    {
                        "substance": s,
                        "amount": initial[s] if n == "pool" else sum(incoming[s]),
                    }
                    for s in substances
                ],
            }
        ...
        rules=rules,
        transfer_bindings=bindings,
        input_bindings=[],
        units=[
            {"substance": s, "unit": "unit", "quantum": quanta[s]} for s in substances
        ],
    )
...
def zero_document():
    return document(
        {"water": 1, "mass": 1},
        {"water": [0], "mass": [0]},
        {s: {"left": [0.5], "right": [0.5]} for s in ["water", "mass"]},
        {"water": 1, "mass": 0.1},
    )
...
def test_desired_zero_carrier_carries_zero_mass():
    ...
    actual = {
        s: [counts(run, b, s)[0] for b in ["left", "right"]] for s in ["water", "mass"]
    }
...
def test_staged_complete_mixing_nonround_recurrence_multiple_constituents():
    water_doc = document(
        {"water": 2},
        {"water": [3, 2, 0]},
        {"water": {"left": [1.5, 2.4, 1], "right": [2.5, 1.6, 0]}},
        {"water": 1},
        3,
    )
    ...
    for t in range(3):
        ...
        for s in initial:
            ...
            for b, v in allocations.items():
                outputs[s][b].append(v)
    ...
    for s in initial:
        for b in released:
            assert counts(mass, b, s) == outputs[s][b]
        assert sum(counts(mass, "pool", s, "outgoing")) + pools[s] == initial[s] + sum(
            inputs[s]
        )
...
def test_staged_float_reentry_does_not_preserve_exact_counts():
    ...
    doc["rules"].append(
        i.rule(
            "sink", "water", i.literal(MERGED_COUNT * QUANTUM), i.release_all("final")
        )
    )
    ...
    doc["projections"]["specifications"].append(
        projection_spec("sink-incoming", "sink")
    )
    doc["projections"]["initial_states"].append(
        {"projection": "sink-incoming", "values": []}
    )
    doc["rules"][-1]["expression"] = i.projection(
        "sink-incoming", value_kind="extensive"
    )
...
def test_same_turn_outgoing_projection_cannot_observe_carrier_allocation():
    ...
    doc["projections"] = {
        "specifications": [spec],
        "initial_states": [{"projection": "carrier-out", "values": []}],
    }
    doc["rules"][-1]["disposition"] = partition(
        {
            "left": i.mul(
                i.literal(0.1), i.projection("carrier-out", value_kind="extensive")
            ),
            "right": i.literal(0),
        }
    )
    ...
    print(
        "SAME_TURN",
        counts(run, "pool", "water", "outgoing"),
        counts(run, "pool", "mass", "outgoing"),
        flush=True,
    )


def coupled_zero_document():
    carrier_doc = document(
        {"water": 1},
        {"water": [0]},
        {"water": {"left": [0.5], "right": [0.5]}},
        {"water": 1},
    )
    ...
    mass_doc = document(
        ...
        {
            "mass": {
                b: [10 * counts(carrier, b, "water")[0] // 1 * 0.1]
                for b in ["left", "right"]
            }
        },
        {"mass": 0.1},
    ...
    mass_doc["forcings"].append(
        {"id": identity, "horizon": {"first": 0, "last": 0}, "values": [0]}
    )
...
def test_staged_zero_carrier_carries_zero_mass_and_binds_identity():
    ...
    mass = model.run(RID)
    actual = {
        "water": [counts(carrier, b, "water")[0] for b in ["left", "right"]],
        "mass": [counts(mass, b, "mass")[0] for b in ["left", "right"]],
    }
    assert actual == {"water": [0, 0], "mass": [0, 0]}
    changed = copy.deepcopy(doc)
    changed["forcings"][-1]["id"] = changed["forcings"][-1]["id"].replace(
        "complete-mixing", "incoming-only"
    )
    other = i.compile_model(changed).run(RID)
    assert other.model_digest != mass.model_digest
    with pytest.raises(ValueError):
        mass.replay_against(other)
    print(
        "STAGED_ZERO",
        actual,
        "IDENTITY",
...
def test_multisubstance_delay_existing_primitives():
    ...
    doc = document(
        {"water": 0, "mass": 0},
        {"water": [10, 0], "mass": [5, 0]},
        {s: {"left": [0, 0], "right": [0, 0]} for s in ["water", "mass"]},
        {"water": 1, "mass": 1},
        2,
    )
    for s in ["water", "mass"]:
        ...
        doc["projections"]["initial_states"].append(
            {"projection": s + "-lag", "values": [{"kind": "extensive", "value": 0}]}
        )
        for rule in doc["rules"]:
            if rule["compartment"] == "pool" and rule["substance"] == s:
                rule["disposition"] = partition(
                    {
                        "left": i.projection(s + "-lag", value_kind="extensive"),
                        "right": i.literal(0),
                    }
                )
    run = i.compile_model(doc).run(RID)
...
def test_dry_inventory_existing_primitives():
    ...
    dry = document(
        ...
        {"water": [0, 4], "mass": [0, 0]},
        {
            "water": {"left": [2, 0], "right": [0, 4]},
            "mass": {"left": [0, 0], "right": [0, 1]},
        },
        {"water": 1, "mass": 1},
        2,
    )
...
def test_single_substance_delay_existing_primitives():
    ...
    doc["projections"] = {
        "specifications": [spec],
        "initial_states": [
            {"projection": "in-lag", "values": [{"kind": "extensive", "value": 0}]}
        ],
    }
    doc["rules"][-1]["disposition"] = partition(
        {"left": i.projection("in-lag", value_kind="extensive"), "right": i.literal(0)}
    )
    run = i.compile_model(doc).run(RID)
    assert counts(run, "left", "water") == [0, 10]
...
@pytest.mark.parametrize(
    "initial_w,initial_m,in_w,in_m,left,right,expected",
    [
...
def test_staged_acceptance_pools(
    initial_w, initial_m, in_w, in_m, left, right, expected
):
    water_doc = document(
        {"water": initial_w},
        {"water": [in_w]},
        {"water": {"left": [left], "right": [right]}},
        {"water": 1},
    )
    ...
    target = {
        b: pool_m * counts(carrier, b, "water")[0] // pool_w for b in ["left", "right"]
    }
    mass_doc = document(
        {"mass": initial_m},
        {"mass": [in_m]},
        {"mass": {b: [v * 0.1] for b, v in target.items()}},
        {"mass": 0.1},
    )
...
def coupled_partition(carrier="water", branches=("left", "right")):
    return {
        ...
        "partition": {
            ...
            "carrier": carrier,
            "branches": [{"branch": b, "carrier_branch": b} for b in branches],
        },
    }
...
def test_public_coupling_complete_pool_recurrence_and_independent_constituents():
    initial = {"water": 2, "salt": 4, "tracer": 11}
    incoming = {"water": [3, 2, 0], "salt": [6, 3, 0], "tracer": [7, 5, 0]}
    outgoing = {s: {"left": [0, 0, 0], "right": [0, 0, 0]} for s in initial}
    outgoing["water"] = {"left": [1.5, 2.4, 1], "right": [2.5, 1.6, 0]}
    doc = document(initial, incoming, outgoing, {s: 1 for s in initial}, 3)
    for rule in doc["rules"]:
        if rule["compartment"] == "pool" and rule["substance"] != "water":
            rule["disposition"] = coupled_partition()
    model = i.compile_model(doc)
    run = model.run(RID)
    ...
    for s in initial:
        assert initial[s] + sum(incoming[s]) == sum(counts(run, "pool", s, "outgoing"))
    assert run.authoritative_log() == model.run(RID).authoritative_log()
    run.replay_against(model.run(RID))


def test_public_proportional_floor_does_not_round_through_f64():
    carrier = 9007199254740629
    dependent = 9007199254739724
    flow = 5198452529468928
    doc = document(
        {"water": carrier, "mass": dependent},
        {"water": [0], "mass": [0]},
        {"water": {"left": [flow], "right": [0]}, "mass": {"left": [0], "right": [0]}},
        {"water": 1, "mass": 1},
    )
    doc["rules"][-1]["disposition"] = coupled_partition()
    run = i.compile_model(doc).run(RID)
    expected = dependent * flow // carrier
    assert expected == 5198452529468405
    assert counts(run, "left", "mass") == [expected]
    assert int(float(dependent) * float(flow) / float(carrier)) != expected


def test_public_subset_same_destination_dry_remainder_and_identity():
    doc = document(
        {"water": 3, "mass": 10},
        {"water": [0, 0], "mass": [0, 0]},
        {
            "water": {"left": [1, 1], "right": [1, 0]},
            "mass": {"left": [0, 0], "right": [0, 0]},
        },
        {"water": 1, "mass": 1},
        2,
    )
    ...
    for binding in doc["transfer_bindings"]:
        if binding["compartment"] == "pool":
            binding["destination"] = "left"
    model = i.compile_model(doc)
    run = model.run(RID)
    assert counts(run, "left", "mass") == [6, 4]
    assert counts(run, "left", "water") == [2, 1]
    reordered = copy.deepcopy(doc)
    reordered["rules"][-1]["disposition"]["partition"]["branches"].reverse()
    assert i.compile_model(reordered).model_digest == model.model_digest
    subset = copy.deepcopy(doc)
    subset["rules"][-1]["disposition"] = coupled_partition(branches=("left",))
    subset["transfer_bindings"] = [
        b
        for b in subset["transfer_bindings"]
        if not (
            b["compartment"] == "pool"
            and b["substance"] == "mass"
            and b["branch"] == "right"
        )
    ]
    subrun = i.compile_model(subset).run(RID)
    assert counts(subrun, "left", "mass") == [3, 7]
    assert subrun.model_digest != run.model_digest
    with pytest.raises(ValueError):
        run.replay_against(subrun)
    # All carrier leaves by two branches, but floor(1/2) per branch retains the mass.
    residue = document(
        {"water": 2, "mass": 1},
        {"water": [0, 0], "mass": [0, 0]},
        {
            "water": {"left": [1, 0], "right": [1, 0]},
            "mass": {"left": [0, 0], "right": [0, 0]},
        },
        {"water": 1, "mass": 1},
        2,
    )
    residue["rules"][-1]["disposition"] = coupled_partition()
    dry = i.compile_model(residue).run(RID)
    assert counts(dry, "pool", "mass", "outgoing") == [0, 0]
    assert counts(dry, "pool", "water", "outgoing") == [2, 0]
